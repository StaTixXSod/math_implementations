...
def solve_linear_equation_QR(matrix) -> list:
    ...
    To get x, you need transform formula above to:
    >>> (QR) @ (QR)^-1 @ x = (QR)^-1 @ b    | * (QR)^-1
    >>> x = R^-1 @ Q.T @ b 
    
    because Q.T = Q^-1 and A@A^-1 = I.

    So, matrix product of Invertible matrix (R^-1), 
    ...
    STEPS:
    ------
    1. Splits original extended matrix into X (original matrix) 
    and y (answer vector)

    2. Get QR matrices

    3. Find coefficient vector
...
def QR_decomposition(A):
    ...
    assert matmul(transpose(Q), Q) == identity_matrix_like(len(A[0]))
...
def orthogonalize_matrix(A):
    ...
    for current_column in range(1, m):
        ...
        for prev_column in range(current_column):
            mult = projection_value(A, U, current_column, prev_column)
            # For each item in this column...
            for i in range(n):
                U[current_column][i] -= mult * U[prev_column][i]
    return U
...
def normalize_matrix(U):
    ...
    for current_column in range(m):
        length = get_vector_norm(U[current_column])
        # For each item in vector...
        for i in range(n):
            Q[current_column][i] = U[current_column][i] / length
    return Q
